...
# ここでは練習問題5(4)の前提に従い、各天気における事前知識がないものとする、
# この場合は、各カテゴリカル変数をインデックスにおきかえ、インデックスごとにパラメータを用意する
# Stanのインデックスは1からなので、1、2、3を使う
# これは実質的に、ダミー変数を使った場合と同等になる
# 練習問題5(4)
# インデックスを使うとサンプリングの時間が伸びる
# attendance_3_dm = pandas.get_dummies(attendance_3)
# print(attendance_3_dm.head())
# 変換用辞書
weather_effec = {
    'A': 1,
    'B': 2,
...
# 実はそちらのほうが推奨。今回は説明変数のインデックス手法による違いのみ比較したかったので変更せず。
...
# 練習問題5(4)でもaucは0.62なので事前知識を導入した場合と変わらずの結果である。
